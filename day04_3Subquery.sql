...
select marka_isim,calisan_sayisi, (select max(maas) from calisanlar where marka_isim=isyeri) as maximum_maas from markalar;
select marka_isim,calisan_sayisi, (select min(maas) from calisanlar where marka_isim=isyeri) as minimum_maas from markalar;

-- max ve min maas bir arada olsun
select marka_isim,calisan_sayisi, (select max(maas) from calisanlar where marka_isim=isyeri) as maximum_maas ,(select min(maas) from calisanlar where marka_isim=isyeri) as minimum_maas
from markalar;
