...
select urun_adi,min(urun_miktari),max(urun_miktari) from manav where urun_adi is not null  group by urun_adi order by urun_adi;
...
select isim,urun_adi, sum(urun_miktari) as toplam_run from manav group by isim,urun_adi order by isim desc;