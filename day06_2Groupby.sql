...
select ulke ,count(ad) from personel group by ulke;
...
select ulke,sehir,count(sehir) as caalisan_sayisi from personel group by sehir,ulke;


-- SORU07: Her ulke için bay ve bayan çalışan sayısı ve yaş ortalamasını sorgulayınız.
select ulke,cinsiyet,count(*) as calisan_sayisi ,avg(yas) as yas_ortalamasi from personel group by ulke,cinsiyet;


-- SORU08: Her ulke için bay ve bayan çalışan sayısı ve yaş ortalamasını  ve maası 30000 den büyük olanları sorgulayınız.
select ulke,cinsiyet,count(*) as calisan_sayisi, avg(yas) as yas_ortalamasi  from personel  where maas > 30000 group by ulke,cinsiyet;
...
select isim,urun_adi, sum(urun_miktari) as top_urun from manav group by isim,urun_adi having top_urun>=3 order by top_urun desc;
